...
export default function Landing() {
  ...
  const handleSignInSubmit = (e) => {
    ...
    const user = {
      id: `user_${Date.now()}`,
      name: userName.trim(),
      email: userEmail.trim() || `${userName.trim().toLowerCase().replace(/\s+/g, '')}@aura.app`,
    ...
    <div className="landing">
      ...
      <div className="landing-inner">
        ...
        <div className="landing-copy animate-fade-up">
          ...
          <p className="landing-fine">Free · No credit card required</p>
        </div>
      ...
      {showAuthModal && (
        <div className="modal-overlay" onClick={() => setShowAuthModal(false)}>
          <div className="modal-box" onClick={e => e.stopPropagation()}>
            <div className="overline" style={{ color: 'var(--primary)', marginBottom: '8px' }}>Sign In</div>
            <h3>Create Your Profile</h3>
            <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem', marginBottom: '20px' }}>
              Enter your preferred name to start chatting with characters.
            </p>
            ...
            <form onSubmit={handleSignInSubmit}>
              ...
              <div className="modal-actions" style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end' }}>
                ...
                <button type="submit" className="btn btn-primary" disabled={loading || !userName.trim()}>
                  {loading ? <span className="spinner" /> : 'Enter Aura →'}
                </button>
              </div>
            </form>
          </div>
